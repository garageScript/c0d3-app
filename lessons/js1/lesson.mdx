...
Recall that challenge 8 takes in 2 numbers, and returns a function that returns
the sum of the 2 numbers when the function is executed.
...
The solutions have been similar up till now; they all got a result from running
a function and displayed it immediately. This time, we had to get a result and
store it until another button was clicked. The browser will store and remember
JavaScript variables until the page is reloaded?but they have to be declared or
defined in the right place!
...
For Challenge 9, write a function that takes in 2 numbers, and returns another
function that takes in a number and returns the sum of all 3 numbers when
called.
