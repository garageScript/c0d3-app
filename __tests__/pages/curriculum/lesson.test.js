          ...
          },
          {
            ...
            challenges: [
              ...
              },
              {
                ...
                title: '2 sum',
                description:
                  'Write a function that takes in an array of numbers and a number, and returns true if any pairs add up to the number. (The numbers in the array is unique, meaning there will be no duplicate numbers)\n```\nsolution([1,2,22,333,23], 25)   // returns true\nsolution([1,2,22,333,23], 24)   // returns true\n```',
                order: 4
              },
              {
                id: '93',
          ...
          },
          {
            ...
            challenges: [
              ...
              },
              {
                ...
                title: 'Meme Gen',
                description:
                  '[Demo Link](https://js5.c0d3.com/imggen)\n\n1. Support a URL called `/api/:text?blur=....` than displays an image with the text etched into the image\n2. Use the [jimp library](https://www.npmjs.com/package/jimp) to help you draw text into the image.',
                order: 3
              },
              {
                id: '70',
                ...
                description:
                  '[Demo Link](https://js5.c0d3.com/assetExercise/)\n\n1. Build a text editing site. The left bar should display a list of all the files that were created.\n2. The right side displays an editor for you to write contents of the file.\n3. The input box on the top lets you specify the file name.',
                order: 4
              },
              {
                id: '69',
                ...
                description:
                  '[Demo Link](https://js5.c0d3.com/chatroom)\n\nAPI URL: https://js5.c0d3.com/auth/api/session\n\n1. User will always see a login / signup screen unless they are logged in.\n2. Add middleware to exchange JWT token for user information so username is stored.\n3. Chatroom should repopulate with new data every 500ms.',
                order: 5
              },
              {
                id: '125',
                ...
                description:
                  '[Demo](https://js5.c0d3.com/auth)\n\nBuild your own backend to support:\n* POST `/api/users` to create a new user\n* POST `/api/sessions` to create a new login.\n* GET `/api/sessions` to get the currently logged in user',
                order: 6
              },
              {
                id: '132',
                title: 'Text detection and search',
                description:
                  '[Demo](https://js5.c0d3.com/ocr.html)\n\n1. Takes pictures\n2. Detect words on the image\n3. Sends POST request to save the image and the word\n4. Search image by word',
                order: 7
              },
              {
                id: '66',
                title: 'Uploading a File From Your Computer to Your Website',
                description:
                  'Create a page with a file input box. Then below that list all the files in your folder. User should be able to upload new files and see the files when they refresh the page.\n\n1. For file uploads, use a [library](https://github.com/expressjs/multer) called `multer`.\n2. To use multer, first create an upload object `upload = multer({ dest: \'files/\' })`\n3. Next, pass your upload object into your post request as a middleware: `app.post(\'/files\', upload.array(\'userFiles\'), (req, res) => ...`\n3. After the middleware, you can access files in the request object: `req.files`\n4. On the front end, make sure your form supports multipart: `<form action="/files" method="POST" enctype="multipart/form-data">`\n5. Make sure your file input field has the same name as what your upload middleware in step 3: `<input type="file" name="userFiles" multiple>`',
                order: 8
              },
              {
                id: '133',
            ...
            challenges: [
              {
                ...
                title: 'GraphQL Intro',
                description:
                  "Create server side graphql endpoint that sends back all the users in the system.\n\n1. Install graphql library and apollo helper library: `yarn add graphql apollo-server-express`\n2. Include library in your server side code: `const { ApolloServer, gql } = require('apollo-server-express');`\n3. Create apollo server object, then apply it your existing app: \n```\nconst server = new ApolloServer({\n  typeDefs: gql(`\n    type Query {\n      users: [String]\n    }\n  `),\n  resolvers: {}\n});\nserver.applyMiddleware({app});\n```\n4. Now if you visit your website, `https://.../graphql`, you will see a playground. Everything you defined in your typeDef will show up there. For now, we only have a query to get users, which is an array of strings (Hint: Look at End2End lesson 4).\n5. To send back data, we define functions in the resolver that either returns the result or returns a promise:\n```\n...\nresolvers: {\n  Query: {\n    users: () => {\n      return ['apple', 'orange', 'banana']\n    }\n  }\n}\n...\n```\n6. To see your graphql server in action, simply go to playground and query for your data: `{users}`. Playground is used for you to quickly check to make sure your server works. \n7. Edit `users.html` so that it sends an ajax request (using fetch) to your graphql endpoint and displays the users so that each user is wrapped in `<h3 />` tag when users visits your site. (ie: `<h3> song </h3> <h3> Joe </h3>`) \n8. Sample fetch request a browser would send to get all users:\n```\nfetch('/graphql', {\n  method: 'POST',\n  headers: {\n    'Content-Type': 'application/json',\n  },\n  body: JSON.stringify({query:'{users}'})\n})\n```",
                order: 1
              },
              {
                id: '193',
                title: 'GraphQL 2',
                description:
                  'Continue from previous problem (make sure its approved and merged in first). Create a simple chatroom using GraphQL backend. You will need a login page (refer to end2end) for users to join with a username and cookies set to the username.\n\nYou can store chatroom messages in memory.\n\n1. Your data response might not always be simple (like an array of Strings). For chat, you would need to return an array of messages, where each message has a name and a value property. To create custom types, change your typeDef:\n```\ngql`\n  type Message {\n    name: String,\n    value: String\n  }\n  type Query {\n    messages: [Message]\n  }\n`\n```\n2. Query is reserved for retrieving data. To create or update data, we should use a mutation. First define the mutation typeDef:\n```\ntype Mutation {\n  addMessage(value: String): Message\n}\n```\n3. Process the data in the resolver:\n```\n... {\n  Query: ...,\n  Mutation: {\n    addMessage: (_, args, context) => {\n      // The second parameter (args), will contain the user input.\n      // Context is a variable you can setup yourself when creating the server. Example below.\n    }\n  }\n}\n...\n```\n4. To test to see if your server is setup correctly, here\'s a sample graphql command: \n```\nmutation {\n  addMessage(value: "ceuhe") {\n    value\n  }\n}\n```\n5. To access the cookie, you would need to get the request object so you can use `req.cookies`. To set up the context, you must define it when creating the Apollo object:\n```\nconst server = new ApolloServer({\n  ...\n  context: ({ req }) => { return { req } }\n```\n6. After setting up the context to have a `req` key, you can now access request object in your resolver in the third parameter:\n```\n    addMessage: (_, args, context) => {\n      // The third parameter (context) will contain a req property\n    }\n```',
                order: 2
              },
              {
                id: '194',
                title: 'React 1',
                description:
                  'Add React JavaScript library and turn Stars UI into a react component called `Stars`\n\n1. First, you want to add react libraries into your html file:\n```\n<script src="https://unpkg.com/react@16/umd/react.development.js"></script>\n<script src="https://unpkg.com/react-dom@16/umd/react-dom.development.js"></script>\n```\n2. React\'s `<component>` tags are not JavaScript, they are merely syntax extension to JavaScript that needs to be compiled into JavaScript that browser understands (ie `document.createElement...`). To do this compilation, we need to add Babel:\n`<script src="https://unpkg.com/babel-standalone@6.15.0/babel.min.js"></script>`\n3. With these 3 scripts, you can now write JSX and include it as a babel template (for babel to parse / compile it)\n`<script type="text/babel" src="/app.js"></script>`\n4. To start, render a component into the DOM:\n```\nReactDOM.render( \n  <h1>Hello World</h1>,\n  document.getElementById(\'root\')\n);\n```\n5. Example of a simple react component:\n```\nclass Star extends React.Component {\n  constructor(props) {\n    super(props); // You must call super whenever you extend\n  }\n  // You can overwrite any of React\'s lifecycle methods.\n  render() { // Whatever you return here will be displayed\n    return (\n      <h2> Hello Stars </h2>\n    )\n  }\n}\n```',
                order: 3
              },
              {
                id: '199',
