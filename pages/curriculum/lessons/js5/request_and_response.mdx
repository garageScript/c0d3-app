...
1. [Save the code here](https://js5.c0d3.com/setup_todo.html) into a html file
  ...
  <Spoiler>
   ...
   > Confused by the variable symbol `$`? When you see something confusing, it
   > is all the more important to stick to your foundations and evaluate **what
   > you know** and make a best guess. In the code, `$` is simply another
   > character in the variable name. There is nothing special about it. The
   > author put `$` in the variable name to help him quickly understand that the
   > variable stores an HTML element.
...
4. To check whether your browser successfully followed the response instruction to set a cookie, go to the API hostname and click on Storage tab. On Chrome it would be Application tab.

    Cookies are set against hostnames. Since you are sending a request to `[https://js5.c0d3.com](https://js5.c0d3.com/auth/api/sessions)`, you must go there first and then look at the cookie.
...
7. When you type Enter into the input box, create a todo item by sending a `POST` request to:
