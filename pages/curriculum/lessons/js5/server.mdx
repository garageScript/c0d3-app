...
If in any steps you are stuck because of a running program in the terminal, try
pressing `Control c` or `Control d` or `Control z` to terminate the process.
...
arguments, a request and a response object, for simplicity. There is actually a
third argument which is a function. This third argument will be covered in the
section below about middleware.
