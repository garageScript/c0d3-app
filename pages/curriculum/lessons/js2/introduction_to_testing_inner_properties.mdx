...
3. Write a function named delayAndCall that takes in 2 parameters, a number and
...
3. **Explanation**
   ...
   - In the `primeMachine` function,
     - Pass in the result from running the `getNextPrime` function as a
       parameter.
     - We name the result from running the `getNextPrime` function to `start` to
   ...
   - Inside the return function,
     - We need additional variable (let's say `result`) that stores the result
       of running `getNextPrime` function.
...
The `<div>` tag is one of the most important HTML tag in modern web design. This
very simple tag `<div>` defines a container or section that is used to group
other elements together inside it. All HTML elements inside the section are
called **child** elements of the `<div>`. For example:
...
we can use `"`, `'`, or `to create a string. We chose` here because it allows us
to easily include line breaks inside the string, and because the HTML is already
using quotation marks. If we had used `"` to begin the string, the first
attribute would have ended it and broken our code.
...
totally replaced with a `<h1>` tag and a new button element. The original button
element has been replaced although the new button has exactly the same class
name and text. It's technically a brand-new element. Replace the current
...
[Read this](https://www.notion.so/garagescript/Run-Code-on-Browser-e73b284698d14af4a008ab9941134a69)
to learn how to open the browser's console.

</div>

</twoColumns>

> For each of the problem below, if you are struggling through the answers,
> please walk through each step with your mentor (in person) or in chatroom to
> make sure you have each step in the answer written correctly.
...
4. We need a function (named display) that takes in a number and returns a
   ...
   - Are you able to figure out how to return just the milliseconds?
     ...
     - Trouble with the math? Make sure you take the time to review
       [these math exercises](https://www.notion.so/garagescript/Math-b1fc634eccd540d6b8ed35cbcd98bf7b)!
...
3. Add a variable called `filterFunction` , we need this to store the result
   from `createFilter`
4. Create a function called `startApp` that:
   ...
   - set the button's onclick function to do this:
     ...
     - Set **Filter** button's onlick function to do this:
       - Run `filterFunction`, store the result in result `h1` element's
         `innerText` property.
...
<ChallengeBar
  href="/curriculum/1"
  description="Complete HTML and testing challenges (which are none currently)"
  title="Master your skill by solving challenges"
/>

export default ({ children }) => (
  <Layout
    lessonCoverUrl={`js-2-cover.svg`}
    title="Arrays"
    lessonUrl="/curriculum/lessons/js2/introduction_to_testing_inner_properties"
    lessonId="1"
    subLessons={[
      'Introduction to testing, inner properties',
