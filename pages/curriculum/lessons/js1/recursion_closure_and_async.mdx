...
3. **Explanation**
   ...
   Inside returned function - We need an \*\*\*\*`if` statement to tell the
   function to return a boolean that checks if `n2` is smaller than `n`
...
You may have noticed, some of the exercises and challenges in JS0 are actually
closure examples.
...
Sometimes when you run a function you want to write a HTML `console.log()`
method to display or print out what is going on the console. Instead of writing
`console.log()` into every function, you can create a function called `addLog`.
When you want to write a function, you pass your function into `addLog` like
this:
...
// Let's say you want to write a function checks if the first number
 ...
 *     return 5
 */
res = addTo3() // 6 (video explanation below)
```
...
2. **Function Shape:** `love` does not take in any parameters, does not return
   anything
...
3. Write a function named countToMe that call `console.log` for every number
...
<Spoiler name="Hint">
  This exercise is very similar to moreHello (Closure, exercise 2). Instead of a
  variable outside the function, here you'll use a default parameter to keep
...
7. Write a function named tryNumRange that takes in a number and a function and
   calls the function with every number from 1 to the input number. If any of
   these return true, return true; if they all return false, return false.
...
3. **Explanation**
   - You are given `x` a value and a function `b`
...
1. Write a function named logNonMatching that `console.log`s every character in
   a word, except for one given character.
...
2. Write a function named logFirstX that logs a given number of characters from
   the _beginning_ of a string.
...
4. Write a function named logOddOnly that prints out only the characters at odd
   indices (remember, index starts at 0) from a string.
...
the oven is done before doing something else? Probably not. You will probably do
other things and and come back to it after the oven finishes cooking the pizza.
Usually, the oven will let you know when it is done (like emitting a beep).
...
export default ({ children }) => (
  <Layout
    lessonCoverUrl={`js-1-cover.svg`}
    title="HTML & Functions"
    lessonUrl="/curriculum/lessons/js1/variables"
    lessonId="2"
    subLessons={['HyperText Markup Language', 'Recursion, closure and async']}
  >
    {children}
  </Layout>
)