...
describe('updateSubmission', () => {
  ...
  test('should update user lesson', async () => {
    ...
    // mock userlesson
    UserLesson.findOne = jest.fn().mockReturnValue(userLesson)
  ...
  test('should notify mattermost', async () => {
    ...
    mattermost.getUserByEmail = jest.fn().mockReturnValue('fakeusername')
    // mock next lesson
    Lesson.findOne = jest
      .fn()
      .mockReturnValue({ order: 2, chatUrl: 'fakeurl/js1-nextfake' })

    mattermost.publicChannelMessage = jest.fn()

    const result = await updateSubmission(submission)

    expect(mattermost.publicChannelMessage).toHaveBeenCalledWith(
      'js0-fake',
